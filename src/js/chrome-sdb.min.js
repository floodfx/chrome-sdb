...
{},true)};f["delete"]=function(a){var b,d,e,c;e=f.find_all();d=[];b=0;for(c=e.length;0<=c?b<c:b>c;0<=c?b++:b--)e[b].get_name()!==a&&d.push(e[b]);return Storage.set("chrome-sdb.profiles",f.profiles_json(d),true)};return f}();var confirm_delete,delete_domain,disable_delete,enable_delete,handle_delete_toggle,handle_query,metadata,profile,query,save_domain,sdb,update_domains_table;if(profile=Profile.primary()){sdb=new SimpleDB(profile);$(function(){return update_domains_table()})}else chrome.tabs.create({url:"config.html"});
update_domains_table=function(f){if(f==null)f=null;return sdb.list_domains(function(a){var b,d,e;e=a.domains;a=function(){var c,k,l;l=[];c=0;for(k=e.length;c<k;c++){d=e[c];b='<button id="metadata_'+d+'" class="btn info" onclick="metadata(\''+d+"')\">metadata</a>";b+=' <button id="delete_'+d+'" class="btn" onclick="confirm_delete(\''+d+'\')" disabled="disabled" style="margin-left:5px">delete</button>';l.push("<tr><td>"+d+"<br />"+b+"</td></tr>")}return l}();$("#domains_table > tbody").html(a.join(""));
f!=null&&f();disable_delete();return $("#confirm_delete_domain_modal").modal("hide")})};enable_delete=function(){$("button[id^=delete_]").removeAttr("disabled").addClass("danger").removeClass("secondary");return $("#domain_deletion_control").text("Disable Delete").addClass("danger").removeClass("secondary").addClass("active")};disable_delete=function(){$("button[id^=delete_]").attr("disabled","disabled").addClass("secondary").removeClass("danger");return $("#domain_deletion_control").text("Enable Delete").addClass("secondary").removeClass("danger").removeClass("active")};
handle_delete_toggle=function(){return $("#domain_deletion_control").hasClass("active")?disable_delete():enable_delete()};
handle_query=function(f){var a,b,d,e,c,k;console.log(f);e=f.items.length;c=f.next_token;if(c!=null){c=c.replace(/\n/g,"");$("#next_page_btn").removeAttr("disabled").attr("onclick","query('"+c+"')")}else $("#next_page_btn").attr("disabled","disabled").removeAttr("onclick");if(e===0){$("#query_results_table > thead").html("<tr><th>Items</th></tr>");return $("#query_results_table > tbody").html("<tr><td>No results...</td></tr>")}else{e=function(){var l,p,o,m;o=f.attr_names;m=[];l=0;for(p=o.length;l<
p;l++){a=o[l];m.push("<th>"+a+"</th>")}return m}();c=function(){var l,p,o,m;o=f.items;m=[];l=0;for(p=o.length;l<p;l++){d=o[l];k=function(){var t,v,r,h;r=f.attr_names;h=[];t=0;for(v=r.length;t<v;t++){a=r[t];b=d.attrs[a];h.push(b!=null?b.length>1?"<td><table><tbody><tr><td>"+b.join("</td><td>")+"</td></tr></tbody></table></td>":"<td>"+b.join("")+"</td>":"<td></td>")}return h}();m.push("<tr><td>"+d.name+"</td>"+k.join("")+"</tr>")}return m}();$("#query_results_table > thead").html("<tr><th>Item Name</th>"+
...
return $("#domain_metadata_modal").modal("show")})};$(function(){return $("input[name=confirm_delete]").keydown(function(f){return $("input[name=confirm_delete]").val()==="DELET"&&f.keyCode===69?$("#confirm_delete_domain_btn").removeAttr("disabled").addClass("danger").removeClass("secondary"):$("#confirm_delete_domain_btn").attr("disabled","disabled").addClass("secondary").removeClass("danger")})});
confirm_delete=function(f){$("#domain_delete_label").html("<h2>Delete "+f+"?</h2>");$("input[name=confirm_delete]").val("");$("#confirm_delete_domain_btn").attr("onclick","delete_domain('"+f+"')");return $("#confirm_delete_domain_modal").modal("show")};delete_domain=function(f){$("#confirm_delete_domain_btn").button("loading");return sdb.delete_domain(f,function(){return update_domains_table(function(){$("#confirm_delete_domain_btn").button("reset");return disable_delete()})})};
save_domain=function(){var f;f=$("#domain_name").val();$("#save_domain").button("loading");return sdb.create_domain(f,function(){return update_domains_table(function(){$("#save_domain").button("reset");return $("#create_domain_modal").modal("hide")})})};})();
