...
add_domains=function(c){var a,b,d,f,e;d=c.length===0?["<tr><td>No domains in this region</td></tr>"]:function(){var k,l,p;p=[];k=0;for(l=c.length;k<l;k++){b=c[k];a='<button id="metadata_'+b+'" class="btn info" onclick="metadata(\''+b+"')\">metadata</a>";a+=' <button id="delete_'+b+'" class="btn" onclick="confirm_delete(\''+b+'\')" disabled="disabled" style="margin-left:5px">delete</button>';p.push("<tr><td>"+b+"<br />"+a+"</td></tr>")}return p}();$("#domains_table > tbody").html(d.join(""));$("#domain_select > option").remove();
e=[];d=0;for(f=c.length;d<f;d++){b=c[d];e.push($("#domain_select").append($("<option>",{value:b}).text(b)))}return e};update_domains_table=function(c){if(c==null)c=null;return sdb.list_domains(function(a){add_domains(a.domains);if(c!=null)return c()})};add_item=function(){$("#domain_select").val(domain_from_query());$("#item_name").val("");$("#attr_name").val("");$("#attr_value_textarea").val("");$("#attr_value_is_multivalued").removeAttr("checked");return $("#add_edit_item_label").text("Add Item")};
