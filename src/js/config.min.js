(function(){var after_intro;after_intro=function(){Storage.set("chrome-sdb-config-intro","true");$("#create_profile_modal").modal("show");return guiders.hideAll()};$(function(){if(Storage.get("chrome-sdb-config-intro")===null)return guiders.createGuider({buttons:[{name:"Close",onclick:after_intro}],description:"Welcome to Simple DB Tool for Chrome.  Please setup one or more AWS Credentials to start using this tool!",id:"first",overlay:true,title:"Chrome Simple DB Tool: Configuration"}).show()});var Storage;
Storage=function(){var a;a={};a.get=function(b,c){var d;if(c==null)c=false;d=localStorage.getItem(b);if(c)d=JSON.parse(d);return d};a.set=function(b,c,d){if(d==null)d=false;if(d)c=JSON.stringify(c);localStorage.setItem(b,c);return c};a.del=function(b){return localStorage.removeItem(b)};return a}();var Profile;
...
{},true)};a["delete"]=function(b){var c,d,e,f,g;e=a.find_all();d=[];c=f=0;for(g=e.length;0<=g?f<g:f>g;c=0<=g?++f:--f)e[c].get_name()!==b&&d.push(e[c]);return Storage.set("chrome-sdb.profiles",a.profiles_json(d),true)};return a}();var Settings;
...
new a(b.access_key,b.secret_key,b.region,b.version,b.https_protocol):null};return a}();var cancel_profile,delete_profile,edit_profile,message,primary,profiles,save_profile,update_profiles_table,use_profile,validate_text_box;primary=Profile.primary();profiles=Profile.find_all();
...
update_profiles_table=function(){var a,b,c,d,e,f,g,i,h;primary=Profile.primary();profiles=Profile.find_all();profiles.length>0&&$("#profiles_table > tbody").html("");h=[];f=0;for(g=profiles.length;f<g;f++){a=profiles[f];d=a.get_name();b=(i=primary!=null?primary.get_name():void 0)!=null?i:null;a=a.get_settings().get_access_key();e=d===b?"In use":$('<a class="btn" name="'+d+'" href="#">Use</a>').click(function(){return use_profile($(this).attr("name"))});c=$('<a href="#" name="'+d+'">Edit</a>').click(function(){return edit_profile($(this).attr("name"))});
b=$('<a href="#" name="'+d+'">Delete</a>').click(function(){return delete_profile($(this).attr("name"))});c=$("<td></td>").append(c);e=$("<td></td>").append(e);b=$("<td></td>").append(b);d=$("<tr><td>"+d+"</td><td>"+a+"</td></tr>").append(c).append(e).append(b);h.push($("#profiles_table > tbody").append(d))}return h};
...
$(function(){$("#save_profile").click(function(){return save_profile()});return $("#cancel_profile").click(function(){return cancel_profile()})});})();
