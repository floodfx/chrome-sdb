...
SimpleDB=function(){function b(a,c){this.profile=a;this.protocol=(this.secure=c!=null?c:false)?"https":"http";this.endpoint=this.profile.get_settings().get_region();this.sdb_base_url=""+this.protocol+"://"+this.endpoint+"?"}b.prototype.set_profile=function(a){return this.profile=a};b.prototype.default_callback=function(a){return console.log(a)};b.prototype.build_request_url=function(a,c){var f,i;c.Action=a;c.Timestamp=AwsUtils.date_time_format();c.AWSAccessKeyId=this.profile.get_settings().get_access_key();
c.Version=this.profile.get_settings().get_version();c.SignatureVersion=1;c.Signature=AwsUtils.generate_sig(c,this.profile.get_settings().get_secret_key());return this.sdb_base_url+function(){var e;e=[];for(f in c){i=c[f];e.push(f+"="+encodeURIComponent(i))}return e}().join("&")};b.regions=function(){return[{name:"US East (Northern Virginia) Region",endpoint:"sdb.amazonaws.com"},{name:"US West (Oregon) Region",endpoint:"sdb.us-west-2.amazonaws.com"},{name:"US West (Northern California) Region",endpoint:"sdb.us-west-1.amazonaws.com"},
{name:"EU (Ireland) Region",endpoint:"sdb.eu-west-1.amazonaws.com"},{name:"Asia Pacific (Singapore) Region",endpoint:"sdb.ap-southeast-1.amazonaws.com"},{name:"Asia Pacific (Tokyo) Region",endpoint:"sdb.ap-northeast-1.amazonaws.com"}]};b.parse_metadata=function(a,c,f){return{meta:{req_id:$("RequestId",a).text(),box_usage:parseFloat($("BoxUsage",a).text()),status:c,req_url:f}}};b.prototype.ajax_request=function(a,c,f,i){if(f==null)f="GET";if(i==null)i=c;return $.ajax({type:f,url:a,success:function(e,
g){return c(b.parse_metadata(e,g,a),e)},error:function(e,g){var d;d=b.parse_metadata(e.responseXML,g,a);d.error={msg:$("Message",e.responseXML).text(),code:$("Code",e.responseXML).text()};return i(d,e.responseXML)},dataType:"xml"})};b.prototype.list_domains=function(a,c,f){var i,e;if(a==null)a=this.default_callback;if(c==null)c=100;if(f==null)f=null;if(__indexOf.call(function(){e=[];for(i=1;i<=100;i++)e.push(i);return e}.apply(this),c)<0)throw"Max domains must be between 1 and 100";c={MaxNumberOfDomains:c};
if(f)c.NextToken=f;return this.ajax_request(this.build_request_url("ListDomains",c),function(g,d){var h;g.error!=null&&a(g);h=[];$("DomainName",d).each(function(){return h.push($(this).text())});g.domains=h;g.next_token=$("NextToken",d).text();return a(g)})};b.prototype.domain_metadata=function(a,c){if(c==null)c=this.default_callback;return this.ajax_request(this.build_request_url("DomainMetadata",{DomainName:a}),function(f,i){f.error!=null&&c(f);f.creation_date_time=$("CreationDateTime",i).text();
f.item_count=parseInt($("ItemCount",i).text());f.item_names_size_bytes=parseInt($("ItemNamesSizeBytes",i).text());f.attribute_name_count=parseInt($("AttributeNameCount",i).text());f.attribute_names_size_bytes=parseInt($("AttributeNamesSizeBytes",i).text());f.attribute_value_count=parseInt($("AttributeValueCount",i).text());f.attribute_values_size_bytes=parseInt($("AttributeValuesSizeBytes",i).text());f.timestamp=$("Timestamp",i).text();return c(f)})};b.prototype.select=function(a,c,f){if(c==null)c=
this.default_callback;if(f==null)f=null;a={SelectExpression:a};if(f)a.NextToken=f;return this.ajax_request(this.build_request_url("Select",a),function(i,e){var g,d,h;i.error!=null&&c(i);h=[];d={};$("Item",e).each(function(){var j;j={attrs:{},name:$("Name:first",$(this)).text()};$("Attribute",$(this)).each(function(){var o,t;o=$("Name",$(this)).text();d[o]=o;t=$("Value",$(this)).text();j.attrs[o]||(j.attrs[o]=[]);return j.attrs[o].push(t)});return h.push(j)});i.items=h;i.attr_names=function(){var j;
j=[];for(g in d)j.push(g);return j}();f=$("NextToken",e).text();if(f!=="")i.next_token=f;return c(i)})};b.prototype.get_attributes=function(a,c,f,i){var e;if(f==null)f=this.default_callback;if(i==null)i=[];c={DomainName:a,ItemName:c};a=0;for(e=i.length;0<=e?a<e:a>e;0<=e?a++:a--)c["AttributeName."+a]=i[a];return this.ajax_request(this.build_request_url("GetAttributes",c),function(g,d){var h;g.error!=null&&f(g);h={};$("Attribute",d).each(function(){var j,o;j=$("Name",$(this)).text();o=$("Value",$(this)).text();
h[j]||(h[j]=[]);return h[j].push(o)});g.attributes=h;return f(g)})};b.prototype.put_attributes=function(a,c,f,i){var e,g,d,h,j,o,t;if(i==null)i=this.default_callback;g={DomainName:a,ItemName:c};h=c=0;for(o=f.length;h<o;h++){a=f[h];e=a.values;j=0;for(t=e.length;j<t;j++){d=e[j];g["Attribute."+c+".Name"]=a.name;g["Attribute."+c+".Value"]=d;if(a.replace&&a.replace===true)g["Attribute."+c+".Replace"]=true;c+=1}}return this.ajax_request(this.build_request_url("PutAttributes",g),function(u){return i(u)})};
b.prototype.delete_attributes=function(a,c,f,i){var e,g,d,h,j,o,t;if(i==null)i=this.default_callback;g={DomainName:a,ItemName:c};h=c=0;for(o=f.length;h<o;h++){a=f[h];if(e=a.values){j=0;for(t=e.length;j<t;j++){d=e[j];g["Attribute."+c+".Name"]=a.name;g["Attribute."+c+".Value"]=d;c+=1}}else{g["Attribute."+c+".Name"]=a.name;c+=1}}return this.ajax_request(this.build_request_url("DeleteAttributes",g),function(u){return i(u)})};b.prototype.batch_delete_attributes=function(a,c,f){var i,e,g,d,h,j,o,t,u,s,
...
d),function(m){return f(m)})};b.prototype.create_domain=function(a,c){if(c==null)c=this.default_callback;return this.ajax_request(this.build_request_url("CreateDomain",{DomainName:a}),function(f){return c(f)})};b.prototype.delete_domain=function(a,c){if(c==null)c=this.default_callback;return this.ajax_request(this.build_request_url("DeleteDomain",{DomainName:a}),function(f){return c(f)})};return b}();var Storage;
